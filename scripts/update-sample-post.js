...
const fullContent = `<h2>Apa Itu Deep Cleaning?</h2>
<p>Deep cleaning adalah proses pembersihan intensif yang menjangkau area-area yang sering terlewat dalam cleaning biasa. Berbeda dengan general cleaning yang fokus pada permukaan visible, deep cleaning mencakup area tersembunyi, noda membandel, dan kotoran yang menumpuk dalam waktu lama.</p>
...
<li><strong>Kosongkan cabinet kecil</strong> - Terutama untuk kitchen dan bathroom cabinets</li>
...
async function main() {
  const existing = await prisma.blogPost.findUnique({
    where: { slug: 'panduan-lengkap-deep-cleaning-rumah-anda' }
  })
  ...
  if (existing) {
    const updated = await prisma.blogPost.update({
      ...
      data: {
        content: fullContent,
        excerpt: 'Deep cleaning bukan sekadar membersihkan permukaan. Pelajari seluk-beluk deep cleaning profesional, kapan perlu melakukannya, dan apa saja yang included dalam layanan ini.',
        readTime: 15,
      }
    })
    console.log('Updated post:', updated.title)
    console.log('ID:', updated.id)
    console.log('Read time:', updated.readTime, 'min')
  } else {
